---
title: "Module Input Document Fully Automated"
format: html
echo: false
warning: false
params: 
  experiment: "Fertilizer"
  taxlevel: Class
  module: "11"
  treatment1: "OF"
  treatment2: "SF"
...
all_genus <- read.csv(here("all_genus.csv"))
```
...
get_correlation_heatmap_2(diffs, taxa_names)
```
...
```{r, echo=FALSE}
library(tictoc)
tic("Code chunk execution")

taxa_list <- unique(dat_cl$Class)
func_samples <- sample_module(taxa_names, 300, taxa_list)

print(paste0("P-value = ", func_samples$pval))

func_samples$samples[4, ] |>
  unlist() |>
  hist(main = paste0("Random Samples of Size ",length(taxa_list)), xlab = "Similarity Metric", col = "skyblue", border = "white")
abline(v = module_function_search2(taxa_names)$metric, col='red', lwd = 3)
